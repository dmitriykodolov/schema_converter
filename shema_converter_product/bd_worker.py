    ...
    def add_data(self):
        ...
        for row in columns:
            ...
            print(f'{row}: {colorama.Fore.GREEN} SUCCESFULLY ADDED!')
                #
                # except Error as e:
                #     print(f'We have a problem: {colorama.Fore.RED} {str.upper(e)}')

# d = DataBaseBuilder('winetime', 'db', 'store.csv', ';')
# d.create_db()
# d.add_data()

def query(db_name, table, query):
    query = f'%{query}%'
...
queries = query('winetime', 'db', 'Leoville')
for i in queries:
    print(i)